...
ip_adresi = 'http://192.168.238.10:81/stream'  # HTTP akış URL'si
    ...
    while not dur.is_set():
        ...
        if contours:
            ...
            if alan > MIN_ALAN:  # Minimum alan sınırı
                ...
                # GUI olmadan çerçeve çizimi kaldırıldı, sadece konsola bilgi yazdır
                merkez_x = x + w // 2
        ...
        # GUI olmadan durum metnini konsola yazdır
        print(f"Durum: {durum_metni}")

        # GUI olmadan döngü devam eder, cv2.waitKey kaldırıldı
        # Programın döngüde çok hızlı çalışmasını önlemek için küçük bir gecikme
